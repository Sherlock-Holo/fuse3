...
use crate::reply::{
    ...
    ReplyDirectoryPlus, ReplyEntry, ReplyLSeek, ReplyLock, ReplyOpen, ReplyPoll, ReplyStatFs,
...
pub trait PathFilesystem {
    ...
    async fn destroy(&self, req: Request) -> Result<()>;
    ...
    async fn symlink(&self, req: Request, path: &OsStr, link_path: &OsStr) -> Result<ReplyEntry> {
        Err(libc::ENOSYS.into())
    }
    ...
    async fn open(&self, req: Request, path: &OsStr) -> Result<ReplyOpen> {
        Err(libc::ENOSYS.into())
    }
    ...
    async fn fsync(&self, req: Request, path: &OsStr, fh: u64, datasync: bool) -> Result<()> {
        Ok(())
    }
    ...
    async fn flush(&self, req: Request, path: &OsStr, fh: u64, lock_owner: u64) -> Result<()> {
        Err(libc::ENOSYS.into())
    }
